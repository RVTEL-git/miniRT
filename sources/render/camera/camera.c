...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/25 19:02:21 by egiraud           #+#    #+#             */
/*   Updated: 2026/03/25 19:10:32 by egiraud          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minirt.h"

static t_mat4	mat4_look_at_fill(t_vec3 forward, t_vec3 right, t_vec3 up,
		t_vec3 orig)
{
	t_mat4	m;
...
t_mat4	mat4_look_at(t_vec3 orig, t_vec3 dir)
{
	...
	m = mat4_look_at_fill(forward, right, up, orig);
	return (m);
}
...
t_ray	generate_ray(t_cam *cm, double x, double y, bool fixed)
{
	...
	ray.dir = vec3_normalize(mat4_apply(cm->trnsf, vec3_set(px, py, 1)));
	return (ray);
}

static t_mat4	build_camera_matrix(t_cam *cam)
{
	t_vec3	orig;
	t_vec3	dir;
